...
# Flask health check server for Uptime Robot monitoring
app = Flask(__name__)
...
API_ID = os.getenv('TELEGRAM_API_ID', '26968163')
API_HASH = os.getenv('TELEGRAM_API_HASH', '39f966287554e996aea17980791a8d9c')
PHONE_NUMBER = os.getenv('TELEGRAM_PHONE_NUMBER', '+966 56 627 3569')
SESSION_STRING = os.getenv('TELEGRAM_SESSION_STRING', '1BJWap1sAUKGOdTphn3Z9BsP8VxbWKFNjIgqJUAFoV9I1wlV0pU2l_lq74zE65XUv70DFsscTAsbsKWF6XykcrPG9bYho9YKf8qW49mlJpCg85mHn2Zfi5tUH5eic-2jfCjjb0cvMYxnqLWt8se7FfuW2UWqS3_0AO5f-UVDN7suIlLbN0idlAvsoSgegm9cEj9HcxT_UyFc7C0tHYOegC1KEBeAxNf0LSN-pS1zC-gBqzyHk2Kxhvzg-kC5TT3BjFDYX_27lK0KeIf-0Gnm-DvJXR9TogRSG6op0VdOkJJykk-VicHvL316w3qmkfvvklAwxO3LnnmBL8Y-YeUo9t8atE3G8pGE=')
...
async def run_forever():
    """Auto-reconnect wrapper for unstable connections (great for Termux/mobile)"""
    restart_count = 0
    ...
    while True:
        try:
            ...
            if restart_count > 1:
                print(f"\n{'='*50}")
                print(f"[♻️] إعادة تشغيل البرنامج... (المحاولة #{restart_count})")
                print(f"{'='*50}\n")
            ...
            print(f"\n[🌐] خطأ في الاتصال ({type(e).__name__}): {e}")
            print("[♻️] إعادة الاتصال بعد 10 ثواني...")
            ...
            print(f"\n[💥] خطأ عام ({type(e).__name__}): {e}")
            print("[♻️] إعادة تشغيل البرنامج بعد 5 ثواني...")
            await asyncio.sleep(5)

# This is the main entry point - run the bot
if __name__ == "__main__":
    print("[🚀] بدء تشغيل البوت مع خاصية إعادة الاتصال التلقائي...")
    print("[📱] مُحسّن للعمل مع Termux على الأندرويد")
    print()
    
    flask_thread = Thread(target=run_flask, daemon=True)
    flask_thread.start()
    print("[✅] تم تشغيل خادم الفحص الصحي على المنفذ 5000")
    print()
